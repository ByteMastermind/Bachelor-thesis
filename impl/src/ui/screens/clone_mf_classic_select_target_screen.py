...
class CloneMFClassicSelectTargetScreen(BaseScreen):
    def __init__(self, command_builder, logger, stacked_widget, file_handler, parent=None):
        ...
        # Init of toasts
        self.add_toast("success", InfoToast(text="Keys successfully set.", duration=1.5, parent=self.stacked_widget))

    def setup_content(self):
        self.description_label = Description("Select target's keys:")
    ...
    def refresh(self):
        ...
        self.load_from_files()
        self.logger.log(f"Updated the {self.settings} files list.", level=logging.INFO)

    def load_from_files(self):
        self.filenames = self.file_handler.get_card_keys(self.settings)
        self.buttons = []
    ...
    def select_file(self, filename):
        self.parent_screen.set_dest_card(filename)
        self.show_toast("success")
        self.logger.log(f"Selected {filename} from {self.settings} files list as target keys.", level=logging.INFO)
