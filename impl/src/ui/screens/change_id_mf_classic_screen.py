...
class ChangeIDMFClassicScreen(BaseScreen):
    ...
    def add_screens_to_stacked_widget(self):
        # Init of screens
        self.change_id_mf_classic_settings_screen = ChangeIDMFClassicSettingsScreen(
            self.command_builder, self.logger, self.stacked_widget, self.file_handler, self)
        self.change_id_mf_classic_input_screen = InputScreen(
        ...
        self.stacked_widget.addWidget(self.change_id_mf_classic_settings_screen)
        self.stacked_widget.addWidget(self.change_id_mf_classic_input_screen)
    ...
    def setup_content(self):
        ...
        self.edit_button = CustomButton("Edit ID", lambda: self.switch_to_screen(self.change_id_mf_classic_input_screen))
        ...
        self.settings_button = CustomButton("⚙️ Settings", lambda: self.switch_to_screen(self.change_id_mf_classic_settings_screen))

        self.print_info()
        self.change_id_button.set_enabled(False)

        self.hbox1.addWidget(self.load_button)
    ...
    def change_id(self):
        self.show_toast("loading")
        if self.settings == constants.GEN1:
            self.run_command(constants.CHANGE_MF_CLASSIC_ID, ' csetuid -u ' + self.card_id)
        elif self.settings == constants.GEN3:
            self.run_command(constants.CHANGE_MF_CLASSIC_ID, ' gen3uid -u ' + self.card_id)

    def react(self, command_name):
        if command_name == constants.CHANGE_MF_CLASSIC_ID:
            self.hide_toasts()
            self.show_toast("write_sent")
        elif command_name == constants.LOAD_MF_ID:
            self.hide_toasts()
            new_id = self.command_builder.get_output()
            if len(new_id) not in (8, 14):
