...
class EmulateIDSelectScreen(BaseScreen):
    def __init__(self, command_builder, logger, stacked_widget, file_handler, parent):
    ...
    def setup_content(self):
        self.description_label = Description(f"Select {self.settings} card:")
    ...
    def load_from_files(self):
        self.cards = self.file_handler.get_card_ids_and_dates(self.settings)
    ...
    def select_id(self, card_id):
        ...
        self.logger.log(f"Selected {card_id} from {self.settings} files list.", level=logging.INFO)

    def set_settings(self, settings):
        self.settings = settings
        self.description_label.setText(f"Select {self.settings} card:")
